...
def sentence_to_grapheme(audio_paths, transcripts, vocab_dest: str = 'D:\workspace\model\my_custom_stt_train\data\vocab'):
    ...
    for transcript in transcripts:
        grapheme_transcripts.append(" ".join(unicodedata.normalize(
            'NFKD', transcript).replace(' ', '|')).upper())
    ...
    with open(os.path.join(f"{vocab_dest}/transcripts.txt"), "w", encoding="utf-8") as f:
        for audio_path, transcript, grapheme_transcript in zip(audio_paths, transcripts, grapheme_transcripts):
            ...
            grpm_id_transcript = sentence_to_target(
                grapheme_transcript.split(), grpm2id)
            f.write(f'{audio_path}\t{transcript}\t{grpm_id_transcript}\n')


def generate_grapheme_labels(grapheme_transcripts, vocab_dest: str = 'D:\workspace\model\my_custom_stt_train\data\vocab'):
    vocab_list = list()
    vocab_freq = list()
    ...
    vocab_freq, vocab_list = zip(
        *sorted(zip(vocab_freq, vocab_list), reverse=True))
    vocab_dict = {
        'id': [0, 1, 2],
        'grpm': ['<pad>', '<sos>', '<eos>'],
        'freq': [0, 0, 0]
    ...
    label_df.to_csv(os.path.join(vocab_dest, "aihub_labels.csv"),
