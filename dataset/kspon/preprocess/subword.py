...
def train_sentencepiece(transcripts, datapath: str = 'D:\workspace\model\my_custom_stt_train\data\kspon\pcm', vocab_size: int = 5000):
...
def sentence_to_subwords(audio_paths: list, transcripts: list, datapath: str = 'D:\workspace\model\my_custom_stt_train\data\kspon\pcm'):
    subwords = list()
    ...
    with open(f'{datapath}/transcripts.txt', 'w') as f:
        for audio_path, transcript in zip(audio_paths, transcripts):
            ...
            f.write(
                f'{audio_path}\t{subword_transcript}\t{subword_id_transcript}\n')
