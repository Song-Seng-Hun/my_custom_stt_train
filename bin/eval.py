...
def inference(config: DictConfig):
    ...
    if config.eval.dataset == 'kspon':
        vocab = KsponSpeechVocabulary(
            f'D:/workspace/model/my_custom_stt_train/data/vocab/aihub_{config.eval.output_unit}_vocabs.csv', output_unit=config.eval.output_unit
        )
    elif config.eval.dataset == 'libri':
        vocab = LibriSpeechVocabulary(
            'D:/workspace/model/my_custom_stt_train/data/vocab/tokenizer.vocab', 'D:/workspace/model/my_custom_stt_train/data/vocab/tokenizer.model')
    else:
        raise ValueError(
            "Unsupported Dataset : {0}".format(config.eval.dataset))
