...
class Checkpoint(object):
    ...
    SAVE_PATH = 'D:/workspace/model/my_custom_stt_train/outputs'
    LOAD_PATH = 'D:/workspace/model/my_custom_stt_train/outputs'
    ...
    def __init__(
            ...
            # current epoch is a loop through the full training data
            epoch: int = None,
    ) -> None:
        self.model = model
    ...
    def save(self):
        ...
        torch.save(trainer_states, os.path.join(
            os.getcwd(), self.TRAINER_STATE_NAME))
    ...
    def load(self, path):
        ...
        """
        path = os.path.join(self.LOAD_PATH, path)
        ...
        if torch.cuda.is_available():
            resume_checkpoint = torch.load(
                os.path.join(path, self.TRAINER_STATE_NAME))
            logger.info(f'resume_checkpoint: {resume_checkpoint}')
            model = torch.load(os.path.join(path, self.MODEL_NAME))

        else:
            resume_checkpoint = torch.load(os.path.join(
                path, self.TRAINER_STATE_NAME), map_location=lambda storage, loc: storage)
            model = torch.load(os.path.join(path, self.MODEL_NAME),
    ...
    def get_latest_checkpoint(self):
        ...
        # sorted_listdir = sorted(os.listdir(os.path.join(
        #     self.LOAD_PATH, checkpoints_path)), reverse=True)
        # return os.path.join(checkpoints_path, sorted_listdir[1])

        sorted_listdir = []
        for (path, dirs, files) in os.walk(self.LOAD_PATH):
