...
class SpectrogramParser(AudioParser):
    ...
    def __init__(
            self,
            # which library to use for feature extraction
            ...
            frame_shift: int = 10,
            # flag indication whether to delete silence or not
            del_silence: bool = False,
            # flag indication whether to reverse input or not
            input_reverse: bool = True,
            # flag indication whether to normalize spectrum or not
            normalize: bool = False,
            # which feature to use [mel, fbank, spect, mfcc]
            transform_method: str = 'mel',
            # hyper Parameter for Freq Masking to limit freq masking length
            freq_mask_para: int = 12,
        ...
        self.spec_augment = SpecAugment(
            freq_mask_para, time_mask_num, freq_mask_num)
        ...
        if transform_method.lower() == 'mel':
            ...
            self.transforms = FilterBank(
                sample_rate, n_mels, frame_length, frame_shift)
        else:
            raise ValueError(
