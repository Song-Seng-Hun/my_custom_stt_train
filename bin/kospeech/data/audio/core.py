...
def load_audio(audio_path: str, del_silence: bool = False, extension: str = 'pcm') -> np.ndarray:
    ...
    try:
        if extension == 'pcm':
            signal = np.memmap(audio_path, dtype='h',
                               mode='r').astype('float32')

            if del_silence:
                non_silence_indices = split(signal, top_db=30)
                signal = np.concatenate([signal[start:end]
        ...
        logger.info('ValueError in {0}'.format(audio_path))
        return None
    except RuntimeError:
        logger.info('RuntimeError in {0}'.format(audio_path))
        return None
    except IOError:
        logger.info('IOError in {0}'.format(audio_path))
        return None
