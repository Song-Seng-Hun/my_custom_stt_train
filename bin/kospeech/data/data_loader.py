...
class SpectrogramDataset(Dataset, SpectrogramParser):
    ...
    def __init__(
            ...
            # flag indication whether to use spec-augmentation of not
    ...
    def get_item(self, idx):
        ...
        feature = self.parse_audio(os.path.join(
            self.dataset_path, self.audio_paths[idx]), self.augment_methods[idx])
...
class MultiDataLoader(object):
    ...
    def __init__(self, dataset_list, queue, batch_size, num_workers, pad_id):
        ...
        for idx in range(self.num_workers):
            self.loader.append(AudioDataLoader(
                self.dataset_list[idx], self.queue, self.batch_size, idx, pad_id))
...
def split_dataset(config: DictConfig, transcripts_path: str, vocab: Vocabulary):
    ...
    if config.train.dataset == 'kspon':
        ...
        raise NotImplementedError(
            "Unsupported Dataset : {0}".format(config.train.dataset))
    ...
    validset = SpectrogramDataset(
        ...
        config=config, spec_augment=True,
        dataset_path=config.train.dataset_path,
        audio_extension=config.audio.audio_extension,
    )
