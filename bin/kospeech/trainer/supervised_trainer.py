...
class SupervisedTrainer(object):
    ...
    TRAIN_RESULT_PATH = "D:/workspace/model/my_custom_stt_train/outputs/train_result.csv"
    VALID_RESULT_PATH = "D:/workspace/model/my_custom_stt_train/outputs/eval_result.csv"
    TRAIN_STEP_RESULT_PATH = "D:/workspace/model/my_custom_stt_train/outputs/train_step_result.csv"
    ...
    def __init__(
            ...
            device: torch.device,                          # device - cuda or cpu
            # number of timesteps to save result after
            print_every: int,
            # nimber of timesteps to save result after
            save_result_every: int,
            # number of timesteps to checkpoint after
            checkpoint_every: int,
            # step of teacher forcing ratio decrease per epoch.
            teacher_forcing_step: float = 0.2,
            # minimum value of teacher forcing ratio
            min_teacher_forcing_ratio: float = 0.8,
            # model to train - las, transformer
            architecture: str = 'las',
            vocab: Vocabulary = None,                      # vocabulary object
            # flag indication whether joint CTC-Attention or not
    ...
    def train(
        ...
        # number of epochs (iteration) for training
        num_epochs: int,
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            teacher_forcing_ratio = max(
                self.min_teacher_forcing_ratio, teacher_forcing_ratio)

            # Validation
            valid_queue = queue.Queue(self.num_workers << 1)
            valid_loader = AudioDataLoader(
                self.validset, valid_queue, batch_size, 0, self.vocab.pad_id)
            valid_loader.start()
            ...
            logger.info(
                'Epoch %d Training result saved as a csv file complete !!' % epoch)
        ...
        while True:
            ...
            if timestep % self.save_result_every == 0:
                self._save_step_result(
                    self.train_step_result, epoch_loss_total / total_num, cer)
    ...
    def _validate(self, model: nn.Module, queue: queue.Queue) -> float:
        ...
        while True:
            ...
            for idx in range(targets.size(0)):
                ...
                predict_list.append(self.vocab.label_to_string(
    ...
    def _model_forward(
        ...
        if architecture == 'las':
            ...
            loss = self.criterion(outputs.transpose(
                0, 1), targets[:, 1:], output_lengths, target_lengths)

        elif architecture in ('rnnt', 'conformer_t'):
            outputs = model(inputs, input_lengths, targets, target_lengths)
            loss = self.criterion(
                outputs, targets[:, 1:].contiguous().int(
            ...
            raise ValueError(
                "Unsupported model : {0}".format(self.architecture))
    ...
    def _save_epoch_result(self, train_result: list, valid_result: list) -> None:
        ...
        train_df.to_csv(SupervisedTrainer.TRAIN_RESULT_PATH,
                        encoding="utf-8", index=False)
        valid_df.to_csv(SupervisedTrainer.VALID_RESULT_PATH,
                        encoding="utf-8", index=False)
    ...
    def _save_step_result(self, train_step_result: dict, loss: float, cer: float) -> None:
        ...
        train_step_df.to_csv(
            SupervisedTrainer.TRAIN_STEP_RESULT_PATH, encoding="utf-8", index=False)
